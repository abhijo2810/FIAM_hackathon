...
def prepare_features(df, factors):
    ...
    print(f"Missing factors: {set(factors) - set(available_factors)}")
...
def select_top_stocks(df, model, available_factors, n=100):
    X = df[available_factors]
    predicted_returns = model.predict(X)
    df['predicted_return'] = predicted_returns
    top_stocks = df.groupby('stock_ticker')['predicted_return'].mean().nlargest(n).index.tolist()
    return top_stocks

def main():
    df = load_data('data_cleaning/hackathon_sample_v2.csv')
    with open('factor analysis/factor_char_list.csv', 'r') as f:
        factors = [line.strip() for line in f if line.strip()]
    
    X, y, available_factors = prepare_features(df, factors)
    ...
    top_100_stocks = select_top_stocks(df, best_model, available_factors)
    pd.Series(top_100_stocks).to_csv('top_100_stocks.csv', index=False, header=['stock_ticker'])
    print("Top 100 stocks saved to 'top_100_stocks.csv'")
